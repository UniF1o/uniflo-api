...
- `Application.status` and `ApplicationJob.status` are kept in sync: `pending → processing → submitted|failed` (plus a transient `action_required` while a run waits in place for student-supplied email-challenge values — see `PortalChallenge`)
...
- **Migration head: `d1e2f3a4b5c6`.** **Always run `alembic upgrade head` immediately after creating a migration or pulling/merging a schema change — never leave a migration unapplied** (an unapplied migration leaves the app and ORM models out of sync → 500s). Chain applied to prod: `b4c3d2e1f0a9` → `c5d4e3f2a1b0` (address split + demographics) → `d6e5f4a3b2c1` (academic_records `record_type`) → `e7f6a5b4c3d2` (Phase 3 portal gap-fill: ~24 `student_profiles` fields incl. mailing-address block, NBT block, `redress_factors` JSONB; new `contacts` + `application_choices` tables) → `f8a7b6c5d4e3` (`applications.popi_consent_at` + `agreement_consent_at` consent timestamps) → `a9b8c7d6e5f4` (new `field_mappings` table — AI-proposed mapping per application for Partner-A's review screen) → `b1a0c9d8e7f6` (new `portal_challenges` table — mid-run email-challenge values, answered via `POST /applications/{id}/challenge`) → `7bd16112db5c` (new `faculties` + `programmes` tables, `universities.scoring_method`) → `e2a1c4b6d8f0` (`programmes.qualification_type` + `programmes.duration_years`) → `c3d2b1a4e5f6` (`programmes.combination` JSONB — major-combination metadata) → `c1856b74cc36` (nullable `programme_id` FK on `applications` and `application_choices`) → `b7c653dd1cee` (new `careers` table with JSONB columns for compensation, employability, subject_rule, recommended_subjects, programme_keywords) → `d1e2f3a4b5c6` (Phase 6/7 `student_profiles` nullable cols: `citizenship_status`/`passport_number`/`study_permit_type` + `subject_choices` JSONB + `guardian_consent_at`/`_by`/`guardian_relationship`).
