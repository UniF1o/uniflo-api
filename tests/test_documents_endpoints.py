...
SIGNED_URL = "https://supabase.co/storage/v1/object/sign/documents/id.pdf?token=sig"
...
def _wire_storage_mocks(mock_get_supabase):
    bucket = mock_get_supabase.return_value.storage.from_.return_value
...
def test_delete_document_success():
    ...
    assert bucket.remove.call_args[0][0] == [
        f"{VALID_USER_ID}/ID_COPY/{VALID_DOCUMENT_ID}.pdf"
    ]
...
def test_delete_document_wrong_owner():
    ...
    mock_document.student_id = uuid.uuid4()
